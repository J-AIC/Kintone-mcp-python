...
FIELD_TOOL_DEFINITIONS: List[Dict[str, Any]] = [
    {
        "name": "add_fields",
        "description": (
            "kintoneアプリにフィールドを追加します。各フィールドには code（フィールドコード）、type（フィールドタイプ）、label（表示名）の指定が必須です。\n"
    ...
    },
    {
        ...
        "description": (
            "選択肢フィールド（ラジオボタン、チェックボックス、ドロップダウン、複数選択）の設定を生成します。\n"
            "フィールドコードに使用できる文字: ひらがな、カタカナ、漢字、英数字、記号(_＿･・＄￥)\n"
            "例: {\n"
